...
- **Crypto-shredding:** each tenant has a DEK; node *payload* (the indexable content + embedding) is sealed with an AEAD; node *structure* (id, edges, timestamps) is cleartext so the DAG stays traversable. Forget(tenant|node) = destroy DEK; structure remains as a tombstone, content is unrecoverable. Federation moves **ciphertext**. **Implementation note (K0 reconciliation, 2026-06-14):** the shipped code (`mem-store/src/shred.rs`) uses **XChaCha20-Poly1305** with a deterministic SIV-style nonce `keyed-blake3(node_id‖plaintext)[..24]` (so Derived rebuilds stay byte-identical) and AAD binding `tenant‖node_id` — a deliberate, documented choice, not AES-256-GCM as earlier drafts stated. The **quantum-safe** boundary is the DEK *key-wrap* (QSSP: Kyber-768 + X25519), staged in `PLANSET/07` §4 (C1); the payload AEAD stays XChaCha20-Poly1305. **ENCRYPT-S1 WP-6 (2026-07-04):** edge **values** and operational-meta **values** (freshness watermark, sync anchors) are now sealed too, under the same per-tenant key (AAD `tenant‖cf‖storage-key`), so a crypto-shred forgets a tenant's relationships and freshness trail along with its nodes; edge/meta storage **keys** stay cleartext (prefix scans by raw node id keep the DAG traversable — accepted topology-visibility residual, rationale in `mem-store/src/shred.rs` module docs). Pre-WP-6 stores are sealed in place by a marker-gated one-shot migration that runs on open (`migrate_seal_v2`).
